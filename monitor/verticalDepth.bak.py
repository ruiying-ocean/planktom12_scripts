...
## Import model data
in_file_diad = Dataset(f"{baseDir}/{tmod}/ORCA2_{freq}_{year}0101_{year}1231_diad_T.nc", "r")
in_file_grid = Dataset(f"{baseDir}/{tmod}/ORCA2_{freq}_{year}0101_{year}1231_grid_T.nc", "r")
in_file_ptrc = Dataset(f"{baseDir}/{tmod}/ORCA2_{freq}_{year}0101_{year}1231_ptrc_T.nc", "r")
...
var_conv_eco = [1e6, 1e6, 1e6, 1e6, 1e6, 1e6]             # units conversion multplier (mol/m2/s -> PgC/yr = 3.78e-7)
...
## Create PFTs plot
var_list_pfts = ["PIC", "FIX", "COC", "MIX", "DIA", "PHA", "BAC", "PRO", "PTE", "MES", "GEL", "MAC"]
var_unit_pfts = ["umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L", "umol/L"]
var_conv_pfts = [1e6, 1e6, 1e6, 1e6, 1e6, 1e6, 1e6, 1e6, 1e6, 1e6, 1e6, 1e6]                         # units conversion multiplier (mol/L -> Pg C = 1.2e-11)
...
obs_alk = np.array((0.00235292, 0.00235541, 0.00235923, 0.00236262, 0.00236528, 0.00236767, 0.0023699, 0.00237204, 0.00237323, 0.00237455, 0.00237551, 0.00237617, 0.00237668, 0.00237678, 0.00237657, 0.00237586, 0.00237447, 0.00237236, 0.00237082, 0.00237223, 0.00238288, 0.00240302, 0.00242312, 0.00243643, 0.00244432, 0.00244916, 0.00245155, 0.00245182, 0.00245059, 0.00245058, np.nan))
