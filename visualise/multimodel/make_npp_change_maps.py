#!/usr/bin/env python3
"""
Generate NPP (PPINT) change maps for each model in an ensemble.

Computes the difference between two time periods:
- Historical: 2000-2010 average
- Future: 2090-2100 average
- Change: Future - Historical

Creates a multi-panel figure with one map per model showing the NPP change.

Usage:
    python make_npp_change_maps.py <output_dir>

    With custom pattern and periods:
    python make_npp_change_maps.py <output_dir> --pattern "TOM12_RY_JRA*" --hist-start 2000 --hist-end 2010 --fut-start 2090 --fut-end 2100
"""
...
def load_period_average(model_dir, model_id, start_year, end_year, plotter):
    """
    Load and compute multi-year average of PPINT (NPP) for a time period.
    ...
    Returns:
        xarray DataArray with time-averaged _PPINT (gC/m²/yr)
    """
    ...
    for year in range(start_year, end_year + 1):
        nc_file = run_dir / f"ORCA2_1m_{year}0101_{year}1231_diad_T.nc"
        ...
        try:
            # Load with preprocessing to get unit-converted _PPINT
            ds = plotter.load_data(str(nc_file), volume=plotter.volume)

            if '_PPINT' not in ds:
                print_warning(f"_PPINT not found in {nc_file}")
                ds.close()
                continue

            # Annual mean for this year
            data = ds['_PPINT'].mean(dim='time_counter')
...
def plot_npp_change_maps(models, output_dir, config,
                         hist_start=2000, hist_end=2010,
                         fut_start=2090, fut_end=2100):
    """
    Create NPP change maps for each model in the ensemble.
    ...
    # Determine grid layout (prefer rows over columns for readability)
    if n_models <= 3:
        n_cols = n_models
        n_rows = 1
    elif n_models <= 6:
    ...
    for model in models:
        ...
        # Try to find any diad_T file
        diad_files = sorted(run_dir.glob("ORCA2_1m_*_diad_T.nc"))
        if diad_files:
            ds = xr.open_dataset(diad_files[0])
    ...
    print_header(f"Loading NPP data for {n_models} models...")
    ...
    for model in models:
        ...
        hist_data = load_period_average(model_dir, model_name, hist_start, hist_end, plotter)
        if hist_data is None:
            model_data[model_name] = None
            continue

        # Load future period
        fut_data = load_period_average(model_dir, model_name, fut_start, fut_end, plotter)
        if fut_data is None:
            model_data[model_name] = None
            continue
    ...
    for idx, model in enumerate(models):
        model_name = model['name']
        model_desc = model.get('desc', model_name)
    ...
    if im is not None:
        ...
        cbar.set_label(f'ΔNPP (g C m⁻² yr⁻¹)\n{fut_start}-{fut_end} minus {hist_start}-{hist_end}',
                       fontsize=12)
        cbar.ax.tick_params(labelsize=10)

    # Add main title
    fig.suptitle(f'Net Primary Production Change\n({hist_start}-{hist_end} → {fut_start}-{fut_end})',
                 fontsize=14, fontweight='bold', y=1.02)

    # Save figure
    output_file = output_dir / f"npp_change_{hist_start}_{hist_end}_to_{fut_start}_{fut_end}.{fmt}"
...
def plot_period_maps(models, model_data, nav_lon, nav_lat, output_dir, config,
                     hist_start, hist_end, fut_start, fut_end):
    """
    Create maps showing absolute NPP for each period (historical and future).
    """
    n_models = len(models)
    ...
    # Get NPP metadata for consistent styling
    meta = get_variable_metadata('_PPINT')
    vmax = meta.get('vmax', 400)
    ...
    for period_name, period_key, start_yr, end_yr in [
        ...
        if im is not None:
            ...
            cbar.set_label('NPP (g C m⁻² yr⁻¹)', fontsize=12)
            cbar.ax.tick_params(labelsize=10)

        fig.suptitle(f'Net Primary Production ({start_yr}-{end_yr} Average)',
                     fontsize=14, fontweight='bold', y=1.02)

        output_file = output_dir / f"npp_{period_name}_{start_yr}_{end_yr}.{fmt}"
        fig.savefig(output_file, dpi=dpi, bbox_inches='tight')
        print_success(f"Created {output_file}")
        plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate NPP change maps for ensemble models'
    )
