...
for l in lines:
	...
	if len(l) > 0 and l[0] != '#':
		...
		if type == 'NSMask': NSMaskFileName = value
...
# additional regions for calculating the N/S ratio
NSMaskFile = glob.glob(NSMaskFileName)
nc_nsmask_id = Dataset(NSMaskFile[0], 'r' )
regions.append(nc_nsmask_id.variables['NORTH'][:].data)  # 55
regions.append(nc_nsmask_id.variables['SOUTH'][:].data)  # 56
...
# Null outputs
nullAnnual  = 0 # adjustment from -1 to account for runs without viruses
nullMonthly = np.array( [ np.zeros((6)) for r in range(0,12)] )
