...
def plot_dcm_depth(
    ...
    """
    Plot the depth of the Deep Chlorophyll Maximum (DCM).

    DCM depth is the depth of maximum total chlorophyll within the top
    `max_depth` metres. Cells where mean chlorophyll is below `min_chl`
    are masked to suppress noisy values in nearly-zero chlorophyll regions.

    Args:
        plotter: OceanMapPlotter instance
        diad_ds: Preprocessed dataset containing '_TChl' (deptht × y × x, µg/m³)
        output_path: Where to save the figure
        max_depth: Maximum depth (m) to search for the DCM (default: 300 m)
        min_chl: Minimum mean chlorophyll (µg/m³) required to plot DCM (default: 0.1)
    """
    if '_TChl' not in diad_ds:
        print("Warning: _TChl not found in diad_ds, skipping DCM depth plot")
        return

    tchl = diad_ds['_TChl']

    if 'deptht' not in tchl.dims:
        print("Warning: _TChl has no deptht dimension, skipping DCM depth plot")
        return

    # Time-average if preprocessing was not applied
    if 'time_counter' in tchl.dims:
        tchl = tchl.mean(dim='time_counter')
    tchl = tchl.squeeze()

    # Restrict search to the top max_depth metres
    tchl_shallow = tchl.where(tchl['deptht'] <= max_depth, drop=True)

    # Depth (m) of maximum chlorophyll – idxmax returns coordinate values
    dcm_depth = tchl_shallow.idxmax(dim='deptht')

    # Mask cells with negligible chlorophyll (noisy DCM in nearly-zero regions)
    mean_chl = tchl_shallow.mean(dim='deptht')
    dcm_depth = dcm_depth.where(mean_chl >= min_chl)

    # Apply land mask
    dcm_depth = plotter.apply_mask(dcm_depth)
    ...
    cbar = fig.colorbar(im, ax=ax, orientation='horizontal', pad=0.05, shrink=0.8)
    cbar.set_label('DCM Depth (m)', fontsize=11)
    ax.set_title('Deep Chlorophyll Maximum Depth', fontsize=13)
